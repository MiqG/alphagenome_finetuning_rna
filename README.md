...
# CRG
sbatch src/scripts/submit_snakemake_slurm.sh 'snakemake --cluster "sbatch --cpus-per-task={threads} --time={resources.runtime} --mem={resources.memory}G --partition={resources.partition} --gres={resources.gres} --parsable" --cluster-status src/scripts/status-sacct.sh --jobs 30 --use-conda -s workflows/06-evaluation/Snakefile --latency-wait 60 --keep-going --rerun-incomplete --rerun-triggers mtime'
```
