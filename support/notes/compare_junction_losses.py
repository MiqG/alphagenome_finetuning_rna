#!/usr/bin/env python3
"""Compare JAX vs. paper-pseudocode junction loss formulations.

Shows cases where the JAX cross_entropy_loss goes negative while the
paper's multinomial_cross_entropy stays non-negative.
"""
...
def run_case(name, pred, target, mask=None):
    ...
    for label, val in [("JAX", jax_loss), ("Paper", paper_loss)]:
        if val < 0:
